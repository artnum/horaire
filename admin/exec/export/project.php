...
$ldap_db = new artnum\LDAPDB(
    $ldapServers,
    !empty($ini_conf['addressbook']['base'])
        ? $ini_conf['addressbook']['base']
        : null
);
...
if (isset($_GET['pid']) && is_numeric($_GET['pid'])) {
    ...
    $query_manager = 'SELECT person_name
        FROM project
        LEFT JOIN person ON project_manager = person_id
...
try {
    ...
    foreach ($values as $row) {
        ...
        if (is_null($per_project[$row['project_reference']]['firstdate'])) {
            $per_project[$row['project_reference']]['firstdate'] = $datetime;
        }
        if (is_null($per_project[$row['project_reference']]['lastdate'])) {
    ...
    if (intval($ini_conf['bexio']['enabled']) != '0') {
        ...
        foreach ($invoices as $invoice) {
            ...
            if (in_array(strval($reference), $bxReferences)) {
                continue;
            }
    ...
    if ($repSt->execute()) {
        while (($repData = $repSt->fetch(PDO::FETCH_ASSOC))) {
            /* bexio integartion */
            if (intval($ini_conf['bexio']['enabled']) != '0') {
                if (in_array(strval($repData['facture_reference']), $bxReferences)) {
                    continue;
                }
            }
            ...
            if ($splitvalue !== 0.0) {
                if (intval($repData['facture_type']) === 1) {
    ...
    $writer->writeSheetRow('Résumé', [
       ...
       '',
       $materielMontant,
       '=IFERROR(D' . $line . '/D' . $coutLine . ',0)',
    ], null, ['string', 'string', 'string', 'price', '0.00 %']);
    $line++;
    $writer->writeSheetRow('Résumé', ['']);
    $line++;
    $workcostLine = $line;
    ...
    ], null, ['string', 'string', 'string', 'price', '0.00 %', 'string']);
    $line++;
    ...
    foreach ($processus as $k => $v) {
        $writer->writeSheetRow('Résumé', [
           ...
           '',
           '=IFERROR(D' . $line . '/D' . $coutLine . ',1)'
        ], ['font-style' => 'italic'], ['string', 'string', '0.00 %', 'price', 'string', '0.00 %']);
        $line++;
        $i++;
    }
    ...
    $writer->writeSheetRow('Résumé', [
       ...
       'HT',
       '',
       '=IFERROR(ABS(SUMIF(Factures!G2:G' . $factureEndLine . ', "Créancier", Factures!D2:D' . $factureEndLine . ')),0)',
       '=IFERROR(D' . $line . '/D' . $coutLine . ',1)',
    ], null, ['string', 'string', 'string', 'price', '0.00 %']);
    $line++;
    $writer->writeSheetRow('Résumé', ['']);
    ...
    $writer->writeSheetRow('Résumé', [
       ...
       'HT',
       '',
       '=IFERROR(D' . $workcostLine . '+D' . $creancierLine . '+D' . $materielMontantLine . ', 0)',
       '1'
    ], ['font-style' => 'bold'], ['string', 'string', 'string', 'price', '0.00 %']);
    $line++;
    $soldLine = $line;
    $writer->writeSheetRow('Résumé', ['Prix Vendu', '', '', $project['price']], null, ['string', 'string', 'string', 'price']);
    $line++;

    $writer->writeSheetRow('Résumé', ['Résultat [CHF]', '', '', '=D'. $soldLine . '-D' .$costLine], null, ['string', 'string', 'string', '0.00;[RED]-0.00']);
    $line++;
    ...
    $writer->writeSheetRow(
        ...
        [
          ...
          'HT',
          '',
          '=IFERROR(ABS(SUMIF(Factures!G2:G' . $factureEndLine . ', "Débiteur", Factures!D2:D' . $factureEndLine . ')), 0)'
      ],
        null,
        ['string', 'string', 'string', 'price']
    );
