...
$stmt = $db->prepare('SELECT *,COALESCE((SELECT CAST(SUM("paiement_amount") AS FLOAT) FROM "paiement" WHERE "paiement_facture" = "facture_id"),0.0) AS facture_paid FROM facture WHERE  facture_paid < "facture_amount" AND facture_deleted = 0');
$stmt->execute();

$xlsRow = 10;
$xlsSheet = $SS->getSheetByName('Créanciers');
$now = new DateTime();
$ldap = $ldap_db->_con();
while(($row = $stmt->fetch(\PDO::FETCH_ASSOC))!==FALSE) {
  if (intval($row['facture_type']) !== 1) { continue; }
  ...
  $xlsSheet->setCellValue("J$xlsRow", intval($rappel));

  $xlsSheet->setCellValue("K$xlsRow", intval($overDue->format('%r%a')));
  ...
  while (($repRow = $repStmt->fetch(\PDO::FETCH_ASSOC)) !== FALSE) {
    ...
    $xlsSheet->setCellValue("G$xlsRow", floatval($row['facture_amount']));
