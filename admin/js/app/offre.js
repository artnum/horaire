...
class AccountingDocUI {
  ...
  renderDocBreadcrumbs(projectId, doc, force = false) {
    return new Promise((resolve, reject) => {
      ...
      if (node.dataset.invalid === "false" && !force) {
        return resolve();
      }
      (() => {
        if (projectId) {
          return AccountingDoc.listByProject(projectId);
        }
        return AccountingDoc.listFromDocument(doc.id);
      })()
        .then((docs) => {
          new Barrier("accountingDocUI").register().then((_) => {
            node.dataset.invalid = "false";
            if (docs.length > 0) {
              this.loadConditionBase(docs[docs.length - 1].id);
            }
            const breadcrumbs = docs.toReversed().map((a) => {
            ...
            switch (docs[0].type) {
              case "offer":
                breadcrumbs.push(
                  '<div class="available" data-action="variant">Variante</div>',
                );
                breadcrumbs.push(
                  '<div class="available" data-action="next">Vers exécution</div>',
                );
                break;
              case "order":
                break;
  ...
  render(doc, lines, project) {
    ...
    this.renderDocBreadcrumbs(project ? project.id : null, doc).then((_) => {
    ...
    new Barrier("accountingDocUI").register().then((_) => {
      ...
      if (doc) {
        switch (doc.type) {
          case "offer":
            document
              .querySelector('account-lines[name="accountingDocContent"]')
              .setState("open");
            break;
          default:
            document
  ...
  loadConditionFinal(docId) {
    this._loadCondition(docId, "final").then((_) => {
      const final = document.querySelector(`account-summary[name="final"]`);
      if (
        final.dataset.id ===
        document.querySelector(`account-summary[name="base"]`)?.dataset.id
      ) {
  ...
  createRenderedEditor(content, value, folded = true) {
    ...
    unfold.addEventListener("click", (event) => {
      if (unfold.dataset.folded === "false") {
        content.lastElementChild.innerHTML = marked.parse(value, {
          gfm: true,
          breaks: true,
        });
        unfold.innerHTML =
        ...
        if (content.lastElementChild.firstElementChild) {
          content.lastElementChild.innerHTML =
            content.lastElementChild.firstElementChild.innerHTML;
        }
        unfold.innerHTML =
          '<span class="material-symbols-outlined">unfold_more</span>';
        unfold.dataset.folded = "false";
      ...
      AccountingCondition.lookup(docId)
        .then((condition) => {
          const node = document.querySelector(
            `account-summary[name="${nodeName}"]`,
          );
          if (node) {
            node.dataset.id = docId;
            condition.content = Object.assign(
              defaultCondition,
              condition.content,
            );
            for (const key in condition.content) {
              node.querySelector(`input[name="${key}"]`).value =
...
window.addEventListener("load", () => {
  ...
  document
    .querySelector("div.breadcrumbs")
    .addEventListener("click", (event) => {
      event.target.classList.add("loading");
      if (event.target.dataset.action === "next") {
        const node = document.querySelector(
          'account-lines[name="accountingDocContent"]',
        );
        AccountingDoc.nextStep(node.id).then((doc) => {
      ...
      if (event.target.dataset.action === "variant") {
        ...
        AccountingDoc.createVariant(node.id).then((doc) => {
          UI.loadDocument(doc.id)
            .then((_) => {
              return UI.renderDocBreadcrumbs(doc.project, null, true);
            })
  ...
  document
    .querySelector('button[name="pdf"]')
    ?.addEventListener("click", (event) => {
      AccountingDoc.msword(
        document.querySelector('account-lines[name="accountingDocContent"]')
          .id,
      )
        .then((pdf) => {
  ...
  document
    ...
    .addEventListener("click", (event) => {
      ...
      data.accountingDocContent.lines = data.accountingDocContent.lines.map(
        (line) => {
          const rawData = document.getElementById(line.id)?.dataset
            .rawLineData;
          if (rawData) {
      ...
      const baseConditionNode = document.querySelector(
        'account-summary[name="base"]',
      );
      const finalConditionNode = document.querySelector(
        'account-summary[name="final"]',
      );
      const baseCondition = {
        docid: baseConditionNode.dataset.id,
        content: {},
      };
      const finalCondition = {
        docid: finalConditionNode.dataset.id,
        content: {},
      };
      ...
      Promise.all([
        (() => {
          AccountingCondition.set(baseCondition).then((_) => {
            if (finalCondition.docid !== baseCondition.docid) {
              AccountingCondition.set(finalCondition).then((_) => {
        ...
        AccountingDoc.updateLines(
          data.accountingDocContent.lines,
          data.accountingDocContent.id,
        ),
      ]).then((_) => {
  ...
  const accDoc = document.querySelector(
    'account-lines[name="accountingDocContent"]',
  );
  ...
  accDoc.installTextarea = function (node, isNotLocked = true) {
    ...
    content.addEventListener("dblclick", (event) => {
      if (content.dataset.readonly === "true") {
        return;
      }
      if (content.dataset.started) {
        return;
      }
  ...
  document
    .querySelector("account-lines")
    .addEventListener("lock-line", (event) => {
      accountingDocLineAPI.update(event.detail).then((x) => {
        accountingDocLineAPI.lock(event.detail.id);
      });
    });
  document
    .querySelector("account-lines")
    .addEventListener("unlock-line", (event) => {
      accountingDocLineAPI.unlock(event.detail.id);
    });

  document
    .querySelectorAll('input[name="tvaPercent"]')
    .forEach((node) => (node.value = getTVA()));
  document
    .querySelectorAll('input[name="rplpValue"]')
    .forEach((node) => (node.value = KAAL.taxes.rplp));
