...
export default class Multiselect {
  ...
  handleChangeEvent(event) {
    this.events.forEach((e) => {
      switch (e.event) {
        case 'change':
          e.listener(event.target.checked)
          break
      }
    })
  }
  ...
  handleClickEvent(event) {
    ...
    if (input.checked) {
      ...
      this.setSelected(
        node.dataset.infer.split(',').map((v) => {
          return { id: v }
        }),
      )
    }
  ...
  handleAddNewEntry(event) {
    ...
    popup.addEventListener('submit', (event) =>
      this.handleAddNewEntrySubmit(event),
    )
  ...
  _renderNode(content) {
    ...
    if (content.group) {
      node.dataset.group = content.group
    }
      ...
      this.api
        ...
        .then((items) => {
          ...
          items
            ...
            .forEach((itemNode) => {
              if (itemNode.dataset.group) {
                if (!groups[itemNode.dataset.group]) {
                  const groupNode = document.createElement('DIV')
                  const groupDetail = groupDefinition[itemNode.dataset.group]
                  groupNode.classList.add('multiselect-group')
                  groupNode.innerHTML = `<div class="multiselect-group-title">${format.escape(groupDetail.name)}</div>`
                  this.container.appendChild(groupNode)
                  groups[itemNode.dataset.group] = groupNode
                }
                groups[itemNode.dataset.group].appendChild(itemNode)
              } else {
                if (!groups['__general__']) {
                  const groupNode = document.createElement('DIV')
                  groupNode.classList.add('multiselect-group')
                  this.container.appendChild(groupNode)
                  groups['__general__'] = groupNode
                }
                groups['__general__'].appendChild(itemNode)
              }
          ...
          if (this.api.isWritable()) {
            ...
            addNewEntry.classList.add(
              'multiselect-item',
              'multiselect-item-add-new',
            )
          ...
          `
            addNewEntry.addEventListener('click', (event) =>
              this.handleAddNewEntry(event),
            )
