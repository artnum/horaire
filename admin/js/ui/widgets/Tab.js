export default class Tab {
  ...
  constructor() {
    ...
    `
    this.container.addEventListener('click', event => this.handleClickButtonEvent(event))
  }

  destroy() {
    this.container.removeEventListener('click', event => this.handleClickButtonEvent(event))
  ...
  removeTab(name) {
    ...
    if (this.currentTab === element) {
      if (this.defaultTab) {
        if (this.tabHeader.has(this.defaultTab)) {
  ...
  addTab(name, label, element, order = 0) {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (!button.parentNode) { this.container.querySelector('.tab-header').appendChild(button) }
    })
  ...
  showTab(name) {
    ...
    if (this.currentTab) {
      ...
      this.tabHeader.get(hideTab.dataset.name).button.setAttribute('aria-selected', 'false')
      window.requestAnimationFrame(() => {
        this.container.querySelector(`[data-action="${hideTab.dataset.name}"]`).classList.remove('active')
    ...
    if (this.currentTab) {
      ...
      this.tabHeader.get(showTab.dataset.name).button.setAttribute('aria-selected', 'true')
      window.requestAnimationFrame(() => {
        this.container.querySelector(`[data-action="${name}"]`).classList.add('active')
