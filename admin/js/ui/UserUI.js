...
export default class UserUI {
        ...
        .then(([prices, tr]) => {
          prices = prices.map((value) => {
            ...
            return {
              ...
              validity: new Intl.DateTimeFormat().format(value.validity),
            };
          });
  ...
  deleteUser(node) {
    ...
    this.userAPI
      ...
      .then((_) => {
        this.app.removeFromNavigation(
          `.item[data-id="${userForm.dataset.id}"]`,
        );
        return this.new();
      })
      .then((_) => {
                ...
                this.app.addMainAction(
                  ...
                  (node) => {
                    this.saveUser().then((user) => {
                      this.putNodeInActiveZone(
                        this._renderNavigationUser(user),
                      );
  ...
  _saveGoups(userid) {
    ...
    UserGroupAPI.setGroups(userid, selected).then((x) => {
      console.log(x);
    });
  }

  _saveAccessRight(userid) {
    ...
    AccessAPI.setUserRoles(userid, selected).then((x) => {
      console.log(x);
    });
  }
          ...
          .then((content) => {
            ...
            this.userAPI
              ...
              .then((list) => {
                ...
                list.forEach((u) => {
                  const node = this._renderNavigationUser(u);
