function UIKABXFactureList() {
  ...
  this.domNode.innerHTML = `
    ...
    `;
  const b = [
    new KAButton("Entrante", {
      value: JSON.stringify({ state: "INCOMING" }),
      samll: true,
      classNames: ["incoming"],
      group: "billtype",
    }),
    new KAButton("Ouverte", {
      value: JSON.stringify({ state: "OPEN" }),
      samll: true,
      classNames: ["open"],
      group: "billtype",
    }),
  ...
  this.payModeState.addEventListener("change", (event) => {
    if (this.payModeState.checked) {
      ...
      new KSelectUI(node, new KAPI(`${KAAL.getBase()}/BXBankAccount`), {
        attribute: "name",
        realSelect: true,
        allowFreeText: false,
      });
      ...
      window.requestAnimationFrame(() =>
        this.selectorNode.appendChild(payButton),
      );
    ...
    window.requestAnimationFrame(() =>
      this.selectorNode.querySelector('button[name="pay"]').remove(),
    );
  ...
  this.listNode.addEventListener("keyup", (event) => {
    ...
    switch (name) {
      ...
      case "search":
        ...
        if (value === "") {
          this.listNode
            .querySelectorAll(".billItem[data-id]")
            .forEach((node) => {
            ...
            value.forEach((word) => {
              ...
              Array.from(node.querySelectorAll("span")).forEach((span) => {
                if (span.textContent.toLowerCase().includes(word)) {
                  ...
                  if (span.dataset.highlight == pass) {
                    span.classList.remove("highlight");
                  }
                }
              });
  ...
  const billid = window.location.search.substring(1);
  if (billid.startsWith("id=")) {
    const KAPIBill = new KAPI(`${KAAL.getBase()}/Facture`);
    KAPIBill.get(billid.substring(3)).then((details) => {
      this.render({ state: details.state }).then((_) => {
        this.openBill(details);
      });
    });
  }
}
    ...
    KAPIBill.search(query)
        ...
        return new Promise((resolve) => {
          ...
          Promise.allSettled(addresses.map((id) => KAPIQRAddress.get(id)))
            ...
            .then((addresses) => {
              resolve(
                bills.map((bill) => {
                  bill.qraddress = addresses.filter(
                    (address) => address.id === bill.qraddress,
                  )[0];
      ...
      .then((openBills) => {
        nodes = openBills.map((bill) => {
          ...
          if (bill.remainder > 0) {
            div.classList.add(`level${bill.remainder}`);
          }
          if (bill.qraddress) {
            div.dataset.address = bill.qraddress.id;
          }
          ...
          div.addEventListener("click", (event) => {
            ...
            Promise.all([this.clearAction(), this.clearPreview()]).then((_) =>
              this.openBill(details),
            );
          });
          return div;
...
UIKABXFactureList.prototype.clearPreview = function () {
  return new Promise((resolve) => {
    window.requestAnimationFrame(() => {
      Array.from(this.previewNode.children).forEach((child) =>
        this.previewNode.removeChild(child),
      );
      resolve();
    });
  });
...
UIKABXFactureList.prototype.clearAction = function () {
  return new Promise((resolve) => {
    window.requestAnimationFrame(() => {
      Array.from(this.actionNode.children).forEach((child) =>
...
UIKABXFactureList.prototype.openBill = function (details) {
  ...
  const KAPIBill = new KAPI(`${KAAL.getBase()}/Facture`);
  Promise.allSettled([this.clearSelectedBill(), this.clearPreview()])
    .then((_) => {
      ...
      if (result.length === 1) {
        ...
        if (fileObject.file !== "") {
          ...
          const url = URL.createObjectURL(
            b64toBlob(fileObject.file, fileObject.mimetype),
          );
          iframe.src = url;
          window.requestAnimationFrame(() => {
  ...
  return new Promise((resolve) => {
    ...
    Promise.allSettled([
      ...
      .then(([bill, address]) => {
        ...
        window.requestAnimationFrame(() =>
          document
            .querySelector(`.billItem[data-id="${bill.id}"]`)
        ...
        if (address.extid === "") {
          ...
          form
            .querySelector('button[name="deleteBill"]')
        ...
        if (!bill.conditions && !address.conditions) {
          bill.conditions = "0:30";
        }
        if (!bill.conditions && address.conditions) {
          bill.conditions = address.conditions;
        }
        if (
          bill.conditions &&
          address.conditions &&
          bill.conditions.charAt(0) !== "!"
        ) {
          bill.conditions = address.conditions;
        }
        const conditions = this.parseCondition(bill.conditions);

        const detailFieldset = KAFieldsetUI(
          "Détail factures",
          this.currentState !== "INCOMING" ? "closed" : "open",
        );
        detailFieldset.innerHTML += `
                ...
                ${conditions
                  .map(
                    (
                      condition,
                    ) => `<div class="condition  ${bill.condition === `${condition[0]}:${condition[1]}` ? "applied" : ""}">
        ...
        const creancierFieldset = KAFieldsetUI(
          "Créancier",
          this.currentState !== "INCOMING" ? "closed" : "open",
        );
        ...
        form.addEventListener("submit", (event) => {
          ...
          KAPIQRAddress.write(address, address.id ?? null).then((address) => {
            ...
            return Promise.all([
              ...
              .then(([updatedBill, file]) => {
                ...
                return KAPIBill.execute("BXCreateBill", {
                  fileuuid: file.data[0].uuid,
                  billid: bill.id,
                });
              })
              .then((_) => {
                return Promise.allSettled(
                  Array.from(
                    form.querySelectorAll("[data-repartition-id]"),
                  ).map((node) => {
                    if (
                      node.querySelector('input[name="value"]').value === ""
                    ) {
                      return Promise.resolve();
                    }
                    const rep = {
                      value: node.querySelector('input[name="value"]').value,
                      project: node.querySelector('input[name="project"]')
                        .dataset.value,
                      facture: bill.id,
                    ...
                    if (isNaN(rep.value) || isNaN(rep.tva)) {
                      return Promise.resolve();
                    }
                    ...
                    return KAPIRepartition.write(
                      rep,
                      node.dataset.repartitionId,
                    );
                  }),
                );
              })
        ...
        form.addEventListener("change", (event) => {
          ...
          switch (target.name) {
            case "date":
              ...
              return (form.querySelector('input[name="duedate"]').value = day
                .toISOString()
                .substring(0, 10));
          }
        });
          ...
          switch (target.name) {
            case "reference":
              return target.setAttribute(
                "aria-invalid",
                QRBill.verify_qr_reference(target.value) ? "false" : "true",
              );
            case "iban":
              return target.setAttribute(
                "aria-invalid",
                QRBill.verify_iban(target.value) ? "false" : "true",
              );
          }
        });
        ...
        form.addEventListener("click", (event) => {
          switch (event.target.name) {
            ...
            case "applyCondition":
              ...
              day.setDate(day.getDate() + parseInt(condition[1]));
              form.querySelector('input[name="duedate"]').value = day
                .toISOString()
                .substring(0, 10);
              ...
              form
                ...
                .forEach((node) => node.classList.remove("applied"));
              event.target.parentNode.classList.add("applied");
              return (form.querySelector('input[name="amount"]').value =
                Math.round(
                  KAFloat(bill.amount * (1 - parseInt(condition[0]) / 100)) *
                    100,
                ) / 100);
          }
        });
        ...
        this.renderAssociate(bill)
          ...
          .then((_) => {
            ...
            if (this.currentState !== "INCOMING") {
              const b = [
                new KAButton("1er Rappel", {
                  value: 1,
                  samll: true,
                  classNames: ["level1"],
                  group: "rappel",
                }),
                new KAButton("2ème Rappel", {
                  value: 2,
                  samll: true,
                  classNames: ["level2"],
                  group: "rappel",
                }),
                new KAButton("3ème Rappel", {
                  value: 3,
                  samll: true,
                  classNames: ["level3"],
                  group: "rappel",
              ...
              b.forEach((x) => {
                if (x.getValue() === bill.remainder) {
                  x.setButton();
                }
                fieldset.appendChild(x);
                x.addEventListener("submit", (event) => {
                  const remainder = event.target.dataset.value;
                  KAPIBill.write({ remainder }, bill.id).then((_) => {
                    const node = document.querySelector(
                      `.billItem[data-id="${bill.id}"]`,
                    );
                    if (node) {
                      window.requestAnimationFrame(() => {
                ...
                x.addEventListener("reset", (event) => {
                  if (b.filter((y) => y.isset()).length !== 0) {
                    return;
                  }
                  KAPIBill.write({ remainder: 0 }, bill.id).then((_) => {
                    const node = document.querySelector(
                      `.billItem[data-id="${bill.id}"]`,
                    );
                    if (node) {
            ...
            if (this.currentState === "OPEN") {
              form
                .querySelector('button[name="payBill"]')
                .addEventListener("click", (event) => {
            ...
            if (this.currentState === "PAID") {
              form
                .querySelector('button[name="openBill"]')
                .addEventListener("click", (event) => {
...
UIKABXFactureList.prototype.renderCreditor = function (
  creancierFieldset,
  creditor,
  newAddress = false,
) {
  if (!creditor.iban && !creditor.name) {
    const node = document.createElement("INPUT");
    new KSelectUI(node, new KAPI(`${KAAL.getBase()}/QRAddress`), {
      realSelect: true,
      allowFreeText: false,
    });
    node.addEventListener("change", (event) => {
      const target = event.target;
      new KAPI(`${KAAL.getBase()}/QRAddress`)
        .get(target.dataset.value)
        .then((address) => {
          const firstField = creancierFieldset.querySelector(
            'input[name="type"]',
          );
          if (firstField) {
          ...
          if (address.type === "S") {
            creancierFieldset.insertAdjacentHTML(
              "beforeend",
              `
    ...
    creancierFieldset
      ...
      .addEventListener("click", (event) => {
        const popup = Admin.popup(
          `<form><input type="hidden" name="type" value="k" />
...
UIKABXFactureList.prototype.parseCondition = function (conditionsStr) {
  if (conditionsStr.charAt(0) === "!") {
    conditionsStr = conditionsStr.substr(1);
  }
  const conditions = conditionsStr
    .split(";")
    .map((condition) => condition.split(":"));
  if (conditions.filter((condition) => condition[0] === "0").length === 0) {
    conditions.push(["0", "30"]);
  }
  conditions.sort((a, b) => parseInt(a[1]) - parseInt(b[1]));
  return conditions;
};

UIKABXFactureList.prototype.editConditions = function (
  conditionsStr,
  billId,
  addressId,
) {
...
UIKABXFactureList.prototype.renderAssociateNode = function (
  parent,
  repartition = null,
) {
  ...
  const rep = {
    value: repartition ? repartition.value : "",
    project: repartition ? repartition.project : "",
    tva: repartition ? repartition.tva : getTVA(this.currentBill.date),
  };
  rep.value = KAFloat(rep.value);
  rep.tva = KAFloat(rep.tva);
  ...
  domNode.innerHTML = `
        ...
        `;
  new KSelectUI(
    domNode.querySelector('input[name="project"]'),
    new STProject("Project", true),
  ...
  domNode.querySelector("span.delete").addEventListener("click", (event) => {
    ...
    this.removeRepartitionNode(domNode).then((_) => {
      if (repartitionId === 0) {
        return Promise.resolve();
      }

      const karepartition = new KAPI(`${KAAL.getBase()}/Repartition`);
      karepartition.get(repartitionId).then((repartition) => {
  ...
  new Promise((resolve) => {
    window.requestAnimationFrame(() => {
      parent.insertBefore(domNode, endButton);
      resolve();
    });
...
UIKABXFactureList.prototype.renderAssociate = function (bill) {
  return new Promise((resolve) => {
    ...
    const karepartition = new KAPI(`${KAAL.getBase()}/Repartition`);
    karepartition.search({ facture: bill.id }).then((repartitions) => {
      if (repartitions.length === 0) {
        return resolve(fieldset);
      }
      if (KAFloat(repartitions[0].splittva) !== 0) {
        fieldset.querySelector('input[name="tva"]').value = KAFloat(
          repartitions[0].splittva,
        );
      }
      ...
      let acc = repartitions
        .map((repartition) => repartition.split === 1)
        .reduce((acc, repartition) => {
...
UIKABXFactureList.prototype.calculateRepartitionTotal = function (fieldset) {
  ...
  if (!this.currentBill._ttc) {
    repAmount =
      nodes.reduce((acc, node) => {
        ...
        value *=
          1 +
          KAFloat(node.parentNode.querySelector('input[name="tva"]').value) /
            100;
        if (!isNaN(value)) {
          acc += value;
        }
      ...
      KAFloat(fieldset.querySelector('input[name="frais"]').value) *
        (1 + KAFloat(fieldset.querySelector('input[name="tva"]').value) / 100);
  }
  ...
  if (this.currentBill) {
    ...
    fieldset.querySelector('input[name="amount"]').value = KAFloat(
      remaining,
      2,
    );
    if (remaining < 0) {
...
UIKABXFactureList.prototype.unselectAllBill = function () {
  this.multiselect = [];
  window.requestAnimationFrame(() => {
    this.listNode.querySelector(".header .total").textContent = "";
  });
  this.listNode
    .querySelectorAll(".billItem.payment")
    .forEach((node) =>
...
UIKABXFactureList.prototype.paySelected = function (bank) {
  ...
  Promise.allSettled(
    this.multiselect.map((item) =>
      KAPIBill.execute("pay", { id: item.id, bank }),
    ),
  )
    .then((results) => {
      return results
    ...
    .then((failed) => {
      return Promise.allSettled(
        failed.map((fail) => KAPIBill.get(fail.__request.params.id)),
      );
    })
    .then((results) => {
      return results
    ...
    .then((bills) => {
      this.clearPreview()
        .then((_) => {
          ...
          const KAPIQRAddress = new KAPI(`${KAAL.getBase()}/QRAddress`);
          bills.forEach((bill) => {
            KAPIQRAddress.get(bill.qraddress).then((qraddress) => {
              const div = document.createElement("DIV");
              div.classList.add("billItem");
              if (bill.duedate === "") {
