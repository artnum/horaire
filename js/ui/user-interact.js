function OldLoginInteface(UserUI) {
    if (OldLoginInteface._instance) { return OldLoginInteface._instance }
    ...
    requestPassword(uid) {
        return new Promise((resolve, reject) => {
            ...
            logForm.querySelector('form').addEventListener('submit', event => {
                ...
                klogin.login(uid, new FormData(event.target).get('motdepasse'))
                .then(token => {
...
function NewLoginInterface(UserUI) {
    if (NewLoginInterface._instance) { return NewLoginInterface._instance }
...
NewLoginInterface.prototype = {
    ...
    render(params) {
        ...
        form.addEventListener('submit', event => {
            ...
            const klogin = new KLogin()
            klogin.getUserid(formdata.get('username'))
            .then(userid => {
...
{
    if (InvitationLoginInterface._instance) { return InvitationLoginInterface._instance }
...
InvitationLoginInterface.prototype = {
    ...
    render(invitation) {
        const klogin = new KLogin()
        klogin.getInvitationInfo(invitation)
        .then(information => {
            const container = document.querySelector('.ka-main-top')
            const form = document.createElement('form')
            form.classList.add('ka-login-form')
            ...
            form.addEventListener('submit', event => {
                ...
                if (!valid
                    || formdata.get('pass1') != formdata.get('pass2')) {
                    return MsgInteractUI('error', valid ? 'Mot de passe différents' : 'Mot de passe invalide')
                }
                const klogin = new KLogin()
                klogin.setInvitationPassword(invitation, formdata.get('pass1'))
                .then(userid => {
    ...
    check_password(password)
    {
        ...
        const sequentialNumbers = /(012|123|234|345|456|567|678|789)/.test(password);
    
        const isLongEnough = password.length >= 8;
        if (!isLongEnough || repeatingSequence || sequentialNumbers) { return false }
        if (
...
UserInteractUI.prototype.render = function ()
{
    return new Promise(resolve => {
        ...
        if (params.has('invitation')) {
            return new InvitationLoginInterface(this).render(params.get('invitation'))
        }
