...
export default class UIKAProjectList {
  constructor(parentNode) {
    ...
    Array.from(this.projectList.querySelectorAll('span')).forEach((element) => {
      element.addEventListener('click', (event) => {
        const header = event.currentTarget
        if (!header.dataset.sorted) {
          asc = true
        } else if (header.dataset.sorted === 'asc') {
          asc = false
        } else {
          asc = true
        }
        ...
        for (
          let node = header.parentNode.firstElementChild;
          node;
          node = node.nextElementSibling
        ) {
          window.requestAnimationFrame(() => {
            delete node.dataset.sorted
          })
        }
  ...
  resetSortHeader() {
    for (
      let node = this.projectListHeader.firstElementChild;
      node;
      node = node.nextElementSibling
    ) {
  ...
  sort(by, asc = true) {
    let i = 0
    const a = Array.from(this.projectList.querySelectorAll(`span.${by}`)).sort(
      (a, b) => {
  ...
  render(projects) {
    return new Promise((resolve) => {
      for (let node = this.projectList.firstElementChild; node; ) {
        ...
        if (
          projects.findIndex(
            (p) => (parseInt(p.id) === parseInt(node.id.split('-')[1])) === -1,
          )
        ) {
          const n = node
          window.requestAnimationFrame(() => {
      ...
      Promise.allSettled(
        projects.map((project) => {
          this.renderProject(project).then((domNode) => {
  ...
  textSearch(text, onlyProject = false) {
    ...
    const expr = KQueryExpr.fromString(text, {
      attribute: ['name', 'reference'],
    })
    const query = { deleted: '--' }
    if (this.selectedState === 'open') {
      query.closed = '--'
    } else if (this.selectedState === 'close') {
      query.closed = '*'
    }
    expr.merge(query)

    if (this.runningSearch) {
      this.runningSearch.abort()
    }
    ...
    Promise.allSettled([
      (() => {
        ...
        return this.searchProjectByClient(
          KQueryExpr.fromString(text, {
            attribute: ['givenname', 'o', 'sn'],
          }).object(),
          this.runningSearch.signal,
        )
      })(),
      (() => {
        if (onlyProject) {
        ...
        return this.searchProjectByManager(
          KQueryExpr.fromString(text, { attribute: ['name'] }).object(),
          this.runningSearch.signal,
        )
      })(),
      ...
      .then(([projectByClient, searchProjectByManager, projects]) => {
        ...
        projectByClient = projectByClient.filter(
          (p) => projects.findIndex((p2) => p2.id === p.id) === -1,
        )
  ...
  toggleProjectFolding(domNode, forceOpen = false) {
    ...
    for (const node of this.projectList.querySelectorAll(
      '.ka-project[data-open="1"]',
    )) {
      if (node === domNode) {
        continue
      }
    ...
    this.loadTravaux(projectId)
      .then((travaux) => {
        return [
          travaux.map((travail) => this.renderTravail(travail)),
          this.renderGroup(travaux),
        ]
      })
      .then(([nodes, groups]) => {
        if (nodes.length === 0) {
        ...
        Promise.all(
          groups.map(
            (group) =>
              new Promise((resolve) =>
                window.requestAnimationFrame(() => {
                  const prevNode = parent.querySelector(
                    `div[data-group="${group.dataset.group}"]`,
                  )
                  if (prevNode) {
          ...
          nodes.forEach((node) => {
            const group = parent.querySelector(
              `div[data-group="${node.dataset.mygroup}"]`,
            )
            ...
            window.requestAnimationFrame(() =>
              parent.insertBefore(node, group.nextElementSibling),
            )
          })
        })
      })
  ...
  renderProject(project) {
    return new Promise((resolve) => {
      ...
      KContactOld.simplifyContact(project.client).then((client) => {
        domNode.innerHTML = `
        ...
        domNode.addEventListener('click', (event) => {
          ...
          while (node) {
            if (node.classList.contains('travail')) {
              return
            }
        ...
        Array.from(domNode.querySelectorAll('button')).forEach((button) => {
  ...
  buttonInteractWithProject(projectId, interaction) {
    switch (interaction) {
      case 'edit':
        return this.editProject(projectId)
      case 'export':
        return this.exportProject(projectId)
      case 'print':
        return this.printProject(projectId)
      case 'close':
      ...
      case 'add-work':
        return this.addEditTravailToProject(projectId)
      case 'offer':
        return this.offerProject(projectId)
    }
  }
  offerProject(projectId) {
    window.open(
      `${KAAL.getBase()}/admin/offre.html?project=${projectId}`,
      '_blank',
    )
  }
  buttonInteractWithTravail(projectId, travailId, interaction) {
    switch (interaction) {
      ...
      case 'print':
        return this.printTravail(projectId, travailId)
      case 'delete':
        return this.deleteTravail(travailId)
    }
  }
  exportProject(projectId) {
    return Admin.getUrl(
      `https://zenix/kexport/export-project/${projectId}`,
    ).then((url) => {
      window.open(url)
    })
  ...
  printProject(projectId) {
    return Admin.getUrl('admin/exec/export/bon.php', { pid: projectId }).then(
      (url) => {
        window.open(url)
      },
    )
  ...
  loadTravaux(projectId) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Travail/_query`, {
        method: 'POST',
        body: JSON.stringify({ project: projectId, closed: '0' }),
      })
        .then((travaux) => {
          Promise.allSettled(
            travaux
              ...
              travaux
                .map((t) => {
                  ...
                  t.status =
                    status[
                      status.findIndex(
                        (s) => parseInt(s.id) === parseInt(t.status),
                      )
                    ]
                  return t
                })
                .map((t) => {
                  if (isStringEmpty(t.group)) {
                    t.group = 'Général'
                  }
                  return t
                }),
      ...
      new Promise((resolve) => {
        window.requestAnimationFrame(() => {
          ...
          this.projectList.insertBefore(
            domNode,
            this.projectListHeader.nextElementSibling,
          )
          return resolve()
        })
      }).then((_) => {
  ...
  searchProjectByManager(query = {}, signal = null) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Person/_query`, {
        method: 'POST',
        ...
        .then((managers) => {
          Promise.allSettled(
            managers.map((manager) => {
              ...
              return kafetch2(`${KAAL.getBase()}/Project/_query`, {
                method: 'POST',
                body: query,
                signal: signal,
              })
            ...
            projects = projects
              ...
              .map((p) => {
                return p.map((project) => {
                  project.manager = managers.find(
                    (m) => m.id === project.manager,
                  )
                  return project
                })
              })
            ...
            Promise.allSettled(
              projects
                ...
                .filter((p, index, array) => {
                  return (
                    array.findIndex((p2) => p2.client === p.client) === index
                  )
                })
                .map((project) => {
                  const parts = String(project.client).split('/')
                  return kafetch2(`${KAAL.getBase()}/Contact/${parts.pop()}`, {
                    signal: signal,
                  })
                }),
            ).then((contacts) => {
              ...
              return resolve(
                projects.map((project) => {
                  project.client = contacts.find((c) =>
                    String(project.client).endsWith(String(c.IDent)),
                  )
                  return project
                }),
              )
  ...
  searchProjectByClient(query = {}, signal = null) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Contact/_query`, {
        method: 'POST',
        body: JSON.stringify(query),
        signal: signal,
      }).then((contacts) => {
        Promise.allSettled(
          ...
          Promise.allSettled(
            projects.map((project) => {
              return kafetch2(`${KAAL.getBase()}/Person/${project.manager}`, {
                signal: signal,
              })
            }),
          ).then((managers) => {
  ...
  search(query = {}, signal = null) {
    return new Promise((resolve, reject) => {
      kafetch2(
        `${KAAL.getBase()}/Project/_query?limit=${this.offset},${this.limit}&sort.ordering=DESC&sort.created=DESC`,
        { method: 'POST', body: JSON.stringify(query), signal: signal },
      )
        .then((projects) => {
          ...
          const clients = projects
            .filter((project, index, array) => {
              ...
              return (
                array.findIndex((p) => p.client === project.client) === index
              )
            })
          ...
          const managers = projects
            .filter((project, index, array) => {
              ...
              return (
                array.findIndex(
                  (p) => parseInt(p.manager) === parseInt(project.manager),
                ) === index
              )
            })
          ...
          Promise.all([
            Promise.allSettled(
              clients.map((client) =>
                kafetch2(`${KAAL.getBase()}/${client}?short=1`, {
                  signal: signal,
                }),
              ),
            ).then((responses) => {
            ...
            Promise.allSettled(
              managers.map((manager) =>
                kafetch2(`${KAAL.getBase()}/Person/${manager}`, {
                  signal: signal,
                }),
              ),
            ).then((responses) => {
            ...
            projects = projects
              .map((p) => {
                p.manager = managers.find(
                  (m) => parseInt(m.id) === parseInt(p.manager),
                )
              ...
              .map((p) => {
                p.client = clients.find((c) =>
                  String(p.client).endsWith(String(c.uid)),
                )
  ...
  closeProject(projectId) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Project/${projectId}`, {
        ...
        .then((_) => resolve())
        .catch((cause) =>
          reject(new Error('Erreur fermeture du project', { cause })),
        )
    })
  }
  /**
  ...
  openProject(projectId) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Project/${projectId}`, {
        method: 'PATCH',
        body: JSON.stringify({ id: projectId, closed: null }),
      })
        .then((_) => this.removeProjectNode(projectId))
        .then((_) => resolve())
        .catch((cause) =>
          reject(new Error('Erreur ouverture du project', { cause })),
        )
    })
  }
  /**
              ...
              .then((_) => {
                ;(() => {
                  if (data.get('deleteBexioProject')) {
                    return kafetch2(
                      `${KAAL.getBase()}/BXProject/${project[0].extid}`,
                      { method: 'DELETE' },
                    )
                  }
                  return Promise.resolve()
  ...
  editProject(projectId) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.getBase()}/Project/${projectId}`)
        .then((project) => {
          if (project.length !== 1) {
            throw new Error('Erreur chargement project')
          }
          project = project[0]
          const popup = window.Admin.popup(
            `
          ...
          window.requestAnimationFrame(() => {
            popup.querySelector('div.contact').appendChild(kaoldcontact.domNode)
          })
          if (!isStringEmpty(project.client)) {
            ...
            kaoldcontact.setResult(project.client).then((_) => {
              window.requestAnimationFrame(() => {
          ...
          const managerSelect = new KSelectUI(
            popup.querySelector('input[name="manager"]'),
            new STPerson(),
          ...
          form.addEventListener('keydown', (event) => {
            Array.from(form.querySelectorAll('label.inerror')).forEach(
              (node) => {
                window.requestAnimationFrame(() =>
                  node.classList.remove('inerror'),
                )
              },
            )
          })
          ...
          form.addEventListener('submit', (event) => {
            ...
            if (bxProjectSelect) {
              project.extid =
                submitter === 'bxcreate'
                  ? '_create'
                  : bxProjectSelect.value == 0
                    ? '_unlink'
                    : bxProjectSelect.value
            }
            ...
            this.checkProjectData(project)
              .then((project) => {
                return kafetch2(`${KAAL.getBase()}/Project/${projectId}`, {
                  method: 'POST',
                  body: JSON.stringify(project),
                })
                  .then((projectResult) => {
                    return kafetch2(
                      `${KAAL.getBase()}/Project/${projectResult[0].id}`,
                    )
                  })
                  .then((project) => {
                    return this.associateProject(project[0])
  ...
  checkProjectData(project) {
    return new Promise((resolve, reject) => {
      ...
      if (isIdEmpty(project.manager)) {
        return reject(new Error('Pas de chef de projet', { cause: 'manager' }))
      }
      if (isStringEmpty(project.reference)) {
        return reject(new Error('Pas de référence', { cause: 'reference' }))
      }
      if (isStringEmpty(project.name)) {
        return reject(new Error('Pas de nom', { cause: 'name' }))
      }
      if (isStringEmpty(project.client) || project.client === 'Contact/null') {
        return reject(new Error('Pas de client', { cause: 'client' }))
      }
      return resolve(project)
    })
  }
  checkTravailData(travail) {
    return new Promise((resolve, reject) => {
      if (isIdEmpty(travail.status)) {
        return reject(new Error('Pas de processus', { cause: 'status' }))
      }
      if (isStringEmpty(travail.reference)) {
        return reject(new Error('Pas de référence', { cause: 'reference' }))
      }
      ...
      if (isStringEmpty(travail.begin)) {
        travail.begin = new Date().toISOString().split('T')[0]
      }
      if (isStringEmpty(travail.end)) {
        travail.end = new Date().toISOString().split('T')[0]
      }
  ...
  addEditTravailToProject(projectId, travailId = null) {
    return new Promise((resolve, reject) => {
      ;(() => {
        ...
        KATravail.getByProject(project.id).then((travaux) => {
          new KAList(
            popup.querySelector('input[name="group"]'),
            new KAGroup(travaux),
          )
        })
        const statusSelect = new KSelectUI(
          popup.querySelector('input[name="status"]'),
          new STProcess(),
          { realSelect: true, allowFreeText: false },
        )
        const form = popup.getElementsByTagName('form')[0]
          ...
          this.checkTravailData(travail)
            .then((travail) => {
              if (travailId !== null) {
                ...
                return kafetch2(`${KAAL.getBase()}/Travail/${travailId}`, {
                  method: 'PUT',
                  body: travail,
                })
              }
              return kafetch2(`${KAAL.getBase()}/Travail`, {
                method: 'POST',
                body: travail,
              })
            })
  ...
  deleteAssociateReservation(travailId) {
    return new Promise((resolve, reject) => {
      kafetch2(`${KAAL.kairos.url}/store/Reservation/_query`, {
        method: 'POST',
        body: { affaire: travailId, deleted: '--' },
      })
        .then((reservations) => {
          return Promise.allSettled(
            reservations.map((r) => {
              kafetch2(`${KAAL.kairos.url}/store/Reservation/${r.id}`, {
                method: 'DELETE',
              })
            }),
          )
  ...
  printTravail(projectId, travailId) {
    return new Promise((resolve) => {
      Admin.getUrl('admin/exec/export/bon.php', {
        pid: projectId,
        travail: travailId,
      }).then((url) => {
        window.open(url)
...
UIKAProjectList.prototype.doSearchEvent = new Debounce(function (
  value,
  onlyProject = false,
) {
