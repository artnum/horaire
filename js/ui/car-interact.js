...
KCarInteractUI.prototype = {
    ...
    return new Promise((resolve, reject) => {
      (() => {
        ...
        return new Promise((resolve, reject) => {
          ...
          KAPICarusage.search({ htime: htime }).then((carusage) => {
        ...
        .then((usage) => {
          return new Promise((resolve) => {
            Promise.all([this.getCars(), this.getDefects()]).then(
              (results) => {
        ...
        .then((content) => {
          ...
          select.addEventListener("change", (event) => {
            this.showCarForm(event.currentTarget);
          });
  ...
  showCarForm: function (select) {
    ...
    if (this.content.usage.id) {
      select.dataset.id = this.content.usage.id;
    }
    (() => {
      return new Promise((resolve) => {
        let node =
          document.querySelector("div.ka-car")?.firstElementChild?.nextSibling;
        if (node === null) {
          return resolve();
        }
        while (node) {
  ...
  save: function (htime, node) {
    return new Promise((resolve, reject) => {
      const car =
        node.querySelector(".ka-car-select").options[
          node.querySelector(".ka-car-select").selectedIndex
        ].value;
      const KAPICarusage = new KAPI(`${KAAL.getBase()}/CarUsage`);

      let km = node.querySelector(".ka-car-km")?.value;
          ...
          return new Promise((resolve) => {
            KAPICarusage.search({ htime: htime })
              .then((carusage) => {
                ...
                return Promise.all(
                  carusage.map((x) => KAPICarusage.delete(x.id)),
                );
              })
              .then((_) => {
                return resolve();
      ...
      const defect =
        node.querySelector(".ka-car-defect").options[
          node.querySelector(".ka-car-defect").selectedIndex
        ].value;
      const kusage = {
        htime: htime,
        car: car,
      ...
      (() => {
        if (kusage.id) {
          return KAPICarusage.write(kusage, kusage.id);
        }
