function KAButton(
  ...
  if (!KAButton.__groups) {
    KAButton.__groups = new Map();
  }
  if (!KAButton.__count) {
    KAButton.__count = 0;
  }
  ...
  if (options.group) {
    if (!KAButton.__groups.has(options.group)) {
      KAButton.__groups.set(options.group, []);
    }
