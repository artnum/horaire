...
TimeInteractUI.prototype.addEventListener = function (
  type,
  listener,
  options = {},
) {
...
TimeInteractUI.prototype.run = function () {
  ...
  this.addEventListener("change-date", (event) => {
    ...
    this.loadFromPlanning(date).then((loaded) => {
...
TimeInteractUI.prototype.loadFromPreviousEntry = function (
  date,
  travaux = [],
) {
  ...
  kafetch2(KAAL.url(`Htime/_query`), {
    ...
    for (const time of times) {
      ...
      const status =
        time._travail?.status || time._project?.status || time.process;
...
TimeInteractUI.prototype.unselectTravail = function () {
  return new Promise((resolve) => {
    ...
    window.requestAnimationFrame(
      () => (head.firstElementChild.innerHTML = "🭭"),
    );
    for (let i = 1; i < container.children.length; i++) {
...
TimeInteractUI.prototype.prependPreviousTimeEntry = function (
  temps,
  container = null,
) {
  return new Promise((resolve) => {
...
TimeInteractUI.prototype.insertPreviousTimeEntry = function (
  temps,
  container = null,
) {
  return new Promise((resolve) => {
...
TimeInteractUI.prototype.createPreviousTimeTotal = function (
  day,
  container = null,
) {
  return new Promise((resolve) => {
...
TimeInteractUI.prototype.findLastTimeEntryForDay = function (
  day,
  container = null,
) {
  if (!container) {
...
TimeInteractUI.prototype.showTimeBox = function (
  opts = { id: null, time: null, remark: null },
) {
  return new Promise((resolve) => {
...
TimeInteractUI.prototype.setTime = function (
  project,
  process,
  travail,
  comment,
