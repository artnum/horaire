...
export class User extends SchemaModel {
  static schema = {
    ...
    keyopt: [val => new UserKeyOpt(val), () => new UserKeyOpt(UserKeyOpt.createDefaults())],
    ...
    childs: [
      val => Array.isArray ? val.map(item => new UserChildData(item)) : [],
      []
    ]
  }
}
...
export class UserAPI extends JAPI {
  ...
  listPrice(userid) {
    return this.API.exec(
      ...
      'listPrice',
      { price: userid }
    ).then(prices => prices.map(price => new UserPrice(price)))
  }
