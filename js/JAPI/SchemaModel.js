...
export default class SchemaModel {
  ...
  static isInteger(value) {
    if (typeof value === 'number' && Number.isFinite(value) && value % 1 === 0) { return true }
    if (typeof value === 'number') { return false }
    if (!SchemaModel.isNumeric(value)) { return false }
  ...
  static createDefaults() {
    ...
    for (const [key, [_, defaultValue]] of Object.entries(this.schema)) {
      defaults[key] = (typeof defaultValue === 'function' && defaultValue.name === '')
        ? defaultValue()
        : defaultValue
    }
    return defaults
  ...
  applyToForm(form) {
    ...
    for (const [key, [convert, defaultValue]] of Object.entries(this.constructor.schema)) {
      if (!this[key]) {
  ...
  fromFormData(formData) {
    ...
    for (const [key, [convert, defaultValue, validate]] of Object.entries(this.constructor.schema)) {
      const isSubmodel = typeof defaultValue === 'function' && defaultValue.name === ''
      if (isSubmodel) {
        this[key] = defaultValue()
        const subSchema = this[key].constructor.schema
        for (const [subKey, [subConvert, subDefault, subValidate]] of Object.entries(subSchema)) {
    ...
    if (Object.keys(this.validationErrors).length > 0) {
      throw new Error('Validation failed: ' + JSON.stringify(this.validationErrors))
    }
  }
  ...
  fromObject(object) {
    for (const [key, [convert, defaultValue]] of Object.entries(this.constructor.schema)) {
      ...
      this[key] = (value === null && isSubmodel)
        ? defaultValue()
  ...
  toJSON() {
    ...
    for (const [key, [convert]] of Object.entries(this.constructor.schema)) {
      ...
      json[key] = (value instanceof SchemaModel) ? value.toJSON() : convert(value)
    }
    return json
  }
