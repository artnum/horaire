...
export class JAPIIterator {
  ...
  next() {
    return new Promise((resolve, reject) => {
      if (this.offset === this.end) { return resolve([]) }
      if (this.offset + this.size <= this.loaded.length) {
        this.offset += this.size
        return resolve(
          this.loaded.slice(this.offset - this.size, this.offset)
        )
      }
  ...
  previous() {
    return new Promise((resolve, _) => {
      if (this.offset === 0) { return resolve([]) }
      const previous = this.offset
      this.offset -= this.size
      if (this.offset < 0) { this.offset = 0 }
      let _size = previous - this.offset
      return resolve(
        this.loaded.slice(this.offset, this.offset + _size)
      )
    })
  }
