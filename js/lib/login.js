...
KLogin.prototype.arrayToB64 = function (array) {
  return btoa(String.fromCharCode(...new Uint8Array(array))).replace(/\+/g, '-').replace(/\//g, '_')
}
...
KLogin.prototype.genPassword = function (password) {
  const getRandomInt = function (min, max) {
    ...
    return (((int[0] - min) % width + width) % width) + min
  }
  return new Promise((resolve, reject) => {
    const outKey = {
      derived: '',
...
KLogin.prototype.setPassword = function (userid, password) {
  return new Promise((resolve, reject) => {
    this.genPassword(password)
      .then(key => {
        return fetch(new URL('.auth/setpassword', this.base), {
          method: 'POST', body:
            JSON.stringify(
