...
import { STProcess, STProject } from '../stores.js'
const stProject = new STProject('Project', true)
const stProcess = new STProcess()

function init_mybm_export() {
  const yearSelect = document.getElementById("myBmExport_year")
  const initialYear = parseInt(yearSelect.firstElementChild.getAttribute("value"))
  for(let i = 1; initialYear + i <= new Date().getFullYear(); i++) {
    const option = document.createElement('option')
    option.setAttribute("value", initialYear + i)
    option.innerText = initialYear + i
    if (initialYear + i == new Date().getFullYear()) {
      option.setAttribute("selected", 1)
    }
    yearSelect.appendChild(option)
  }
  const monthSelect = document.getElementById("myBmExport_month")
  const currentMonth = new Date().getMonth() + 1
  for(let i = monthSelect.firstElementChild; i; i = i.nextElementSibling) {
      if (parseInt(i.getAttribute("value")) === currentMonth) {
        i.setAttribute("selected", 1)
        break
      }
  }
  const sendButton = document.createElement('BUTTON')
  sendButton.innerText = "MyBM"
  sendButton.addEventListener("click", event => {
    event.preventDefault()
    const begin = new Date(parseInt(yearSelect.value), parseInt(monthSelect.value) - 1, 1, 12, 0, 0)
    const end = new Date(parseInt(yearSelect.value), parseInt(monthSelect.value), 1, 12, 0, 0)
    end.setTime(end.getTime() - 86400000)
    const beginDate = begin.toISOString().split('T')[0]
    const endDate = end.toISOString().split('T')[0]
    Admin.getUrl('/kexport/export-mybm', {start: beginDate, end: endDate})
    .then(url => {
      window.open(url)
    })
  })
  document.getElementById("myBmExport").appendChild(sendButton)

  const worksheetButton = document.createElement('BUTTON')
  worksheetButton.innerText = "Feuille d'heures"
  worksheetButton.addEventListener("click", event => {
    event.preventDefault()
    const begin = new Date(parseInt(yearSelect.value), parseInt(monthSelect.value) - 1, 1, 12, 0, 0)
    const end = new Date(parseInt(yearSelect.value), parseInt(monthSelect.value), 1, 12, 0, 0)
    end.setTime(end.getTime() - 86400000)
    const beginDate = begin.toISOString().split('T')[0]
    const endDate = end.toISOString().split('T')[0]
    Admin.getUrl('/kexport-test/export-hours', {start: beginDate, end: endDate})
    .then(url => {
      window.open(url)
    })
  })
  document.getElementById("myBmExport").appendChild(worksheetButton)
}
...
window.syncToFrom = syncToFrom
window.editEntry = editEntry
window.deleteEntry = deleteEntry
window.undeleteEntry = undeleteEntry
